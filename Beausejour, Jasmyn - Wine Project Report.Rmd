...
In this model, $\mu$ is the average rating in the training set, $\hat{b}_c$ is the country effect that takes into account the fact that some countries tend to be rated higher, $\hat{b}_t$ is the taster effect that takes into account the fact that some tasters tend to give higher ratings, $\hat{b}_v$ is the vintage effect that takes into account the fact that some vintage tend to be rated higher than others, at least in this dataset. Finally, $\varepsilon_{c,t,v}$ is the residual when using only this linear model.

Once that model is optimized, we will try to minimize $\varepsilon_{c,t,v}$ by using machine learning algorithms such that:

$$
\varepsilon_{c,t,v}=Z_{c,t,v}+\varepsilon_{z,c,t,v}
$$

where $\varepsilon_{z,c,t,v}$ is what we are striving to minimize and $Z_{c,t,v}$ is the contribution of our machine learning algorithms.
...
We can now try to estimate each observation of the test set using this country effect. To be clear, this is the model we are trying, where $\mu$ is the average rating, $b_c$ is the movie_effect and $\varepsilon_{c,t,v}$ is the error term:
  
$$
Y_{c,t,v} = \mu + b_c + \varepsilon_{u,i}
$$
```{r}
...
To be clear, this is the model we are trying, where $\mu$ is the average rating, $\hat{b}_c$ is the regularized_movie_effect, $\hat{b}_t$ is the user-specific effect and $\varepsilon_{c,t,v}$ is the error term:
  
$$
Y_{c,t,v} = \mu + \hat{b}_c +\hat{b}_t + \varepsilon_{c,t,v}
$$
...
Therefore, we need to add avariable to our **Ratings_ML** set that is our current $\varepsilon_{c,t,v}$.
...
train_set <- train_set %>% mutate(epsilon = points - predicted_ratings_train)
test_set <- test_set %>% mutate(epsilon = points - predicted_ratings)

```

#####Machine Learning Algorithms

Below, we train linear regression to minimize the errors when predicting **epsilon**.

```{r}

glm_trained <- train(epsilon ~ price+Lenght+lexical_diversity+lexical_density+repetition+large_word_count+adjectives_count+noun_count+verb_count+good_words, method="glm", data = train_set )

```

Let's now test wether we can improve our results.

```{r}
glm_predicted_ratings <- predicted_ratings+predict(glm_trained,test_set)
...
Great! Our current RMSE is now `r paste(round(100*(naiveRMSE-glm_RMSE)/naiveRMSE,2),"%",sep="")` better than our "Just the average" RMSE.
